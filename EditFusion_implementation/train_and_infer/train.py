...
if __name__ == "__main__":
    ...
    seed_everything()
    script_path = Path(os.path.dirname(os.path.abspath(__file__)))  # 读取数据
    
    # Only have the main process generate the date string
    if accelerator.is_main_process:
        date = time.strftime("%m-%d-%H:%M:%S", time.localtime())  # 获取当前时间
    else:
        date = ""
    
    # Synchronize the date string across all processes
    if accelerator.is_main_process:
        # Write the date to a temporary file
        with open("temp_date.txt", "w") as f:
            f.write(date)
    
    # Make sure the main process has finished writing the file
    accelerator.wait_for_everyone()
    
    # All processes read the date from the file
    ...
    # dataset_name = "100+stars_token64"
    # dataset_name = "100+stars_sample_token32"
    dataset_name = "codebert_mergebert_all_lang"
    # dataset_name = "graphcodebert_mergebert_all_lang"
    # dataset_name = "distilRoBERTa_mergebert_all_lang"
    # dataset_name = "2000_repos_xcolab_removed_token32.csv"
    # dataset_name = "codebert"
    ...
    if model_type == 'gru':
        ...
        model = LSTMClassifier(
            # **model_params, CCEmbedding_class=PretrainedCCEmbedding_linear
            # **model_params, CCEmbedding_class=PretrainedCCEmbeddingSeq
    ...
    train_res_df = trainer.train(model_output_path=model_output_path, date_str=date)  # 训练

    # 只有 main process 才返回结果
